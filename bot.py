#!/usr/bin/env python3
"""XAU/USD Signal Bot v4 — Majburiy 2x kunlik signal"""
...
def fmt_signal(r: dict) -> str:
    ...
    return (
        f"{guar}"
        f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
        f"<b>🥇 XAU/USD · {r.get('entry_tf','5M')} · SIGNAL</b>\n"
        f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
        f"<b>Yo'nalish:</b> {emoji}\n"
        f"<b>Kuch:</b> {stars} {r['strength']}%\n\n"
        f"<b>📊 Trend:</b>  1H {tf_icon(t1h)}{t1h}  ·  5M {tf_icon(t5m)}{t5m}\n\n"
        ...
        f"<b>⚖️ R:R:</b> 1:{r['rr_ratio']}  ·  Lot: {r.get('lot_min','0.01')}–{r.get('lot_max','0.03')}\n\n"
        f"<b>📦 ICT zona:</b> <i>{r.get('ict_note','—')}</i>\n"
        f"<b>🔑 Kalit daraja:</b> <i>{r.get('key_level','—')}</i>\n\n"
        f"<b>🔍 Tahlil:</b>\n<i>{r.get('analysis','—')}</i>\n\n"
        f"<b>❌ Bekor sharti:</b> <i>{r.get('invalidation','—')}</i>\n\n"
        f"<b>⏰</b> {r['timestamp']}\n"
        f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
...
async def main():
    ...
    await send(
        "🤖 <b>XAU/USD Signal Bot v4 — FAOL!</b>\n\n"
        "📊 Juft: 🥇 XAU/USD\n"
        "⏱ TF: 5M/1M | Lot: 0.01–0.03\n\n"
        "🔔 <b>Kafolatlangan signallar:</b>\n"
        "   ⏰ 09:00 — London sessiyasi\n"
        "   ⏰ 15:00 — New York sessiyasi\n\n"
        "📦 ICT: Order Block + FVG + Trend\n"
        "⚡ TP hit → Bez ubytok tavsiyasi\n"
        "📈 Kunlik hisobot: 23:00"
    )
    s = AsyncIOScheduler(timezone=TIMEZONE)
    s.add_job(job_signal, "interval", minutes=5)
    s.add_job(job_tp_sl,  "interval", minutes=2)
